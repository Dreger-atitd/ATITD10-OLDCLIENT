...
function checkWindowSize()
  if not window_check_done_once then
    ...
    local pos = 0; --findText("Useable by")
    if pos then
...
function promptFlaxNumbers()
  ...
  while not is_done do
    ...
    if not is_plant then
      lsPrint(10, y, z, scale, scale, 0xFFFFFFff, "Seeds per harvest:")
      seeds_per_harvest = readSetting("seeds_per_harvest", seeds_per_harvest)
      is_done, seeds_per_harvest = lsEditBox("seedsperhar", 145, y, z, 50, 0, scale, scale, 0x000000ff, seeds_per_harvest)
      ...
      writeSetting("seeds_per_pass", seeds_per_harvest)
      y = y + 27
    end
    ...
    disableRip = readSetting("disableRip", disableRip)
    disableRip = CheckBox(120, y + 59, z + 10, 0xFFFFFFff, " Disable Ripping out by roots", disableRip, 0.7, 0.7)
    writeSetting("disableRip", disableRip)
    y = y + 2

    seedAndWeed = readSetting("seedAndWeed", seedAndWeed)
    seedAndWeed = CheckBox(120, y + 69, z + 10, 0xFFFFFFff, " Cycle between harvesting seeds and flax", seedAndWeed, 0.7, 0.7)
    writeSetting("seedAndWeed", seedAndWeed)
    if seedAndWeed then
        is_plant = false;
        writeSetting("is_plant", is_plant);
    end
    ...
    if is_plant then
      ...
      lsPrintWrapped(10, y+30, z + 10, lsScreenX - 20, 0.7, 0.7, 0xffff40ff, 'Uncheck "Grow Flax" for SEEDS!')
      y = y + 34
      lsPrintWrapped(
        10,
        y + 7,
...
function getPlantWindowPos()
  srReadScreen()
  local plantPos = srFindImage("plant.png"); --findText(seedType)
  if plantPos then
    plantPos[0] = plantPos[0] + 10
    plantPos[1] = plantPos[1] + 5
  else
...
function doCycle(num_loops)
  ...
  for loop_count = 1, num_loops do
    ...
    if not seedAndWeed or is_plant then
      harvestAll(loop_count)
    else
      harvestAll(seeds_per_pass);
    end
    if is_plant and (water_needed or rot_flax) then
      walkTo(water_location, false)
    ...
    if rot_flax and is_plant and (loop_count % 2 or loop_count >= num_loops or not seedAndWeed) then
      rotFlax()
    end
    walkHome(flaxHomePosition)
    drawWater()
...
function rotFlax()
  ...
  --end
  --clickText(pos)
  local invFlax = srFindImage("inventFlax.png");
  if not invFlax then return; end
  srClickMouseNoMove(invFlax[0]+3, invFlax[1]+2);
  lsSleep(refresh_time);
  srReadScreen();
  local pos = srFindImage("rotFlax.png"); --findText("Rot flax")
  if pos then
    --clickText(pos);
...
function plantHere(xyPlantFlax, y_pos)
  ...
  lsSleep(85);
  pinWindow(bed[0], bed[1], PIN_BOTTOM_RIGHT);

--JTJTJT
  if not disableRip then
    srClickMouse(bed[0]+3, bed[1]+3, 1);
    lsSleep(85);
    srReadScreen();
    local utility = srFindImage("utility.png");
    if not utility then
      local escape = "\27";
      srKeyEvent(escape);
      lsSleep(35);
      ...
      lsSleep(85);
      srReadScreen();
      utility = srFindImage("utility.png");
    end
    if not utility then
      return false;
    end
    srClickMouse(utility[0]+3, utility[1]+3);
    lsSleep(75);
    srClickMouse(utility[0]+6, utility[1]+6, 1);
    lsSleep(75);
    safeDrag(utility[0]+6, utility[1]+6, 3, xyWindowSize[1] - 3);
    srClickMouse(bed[0]-6, bed[1]-6, 1); -- clears unclosed window..
  end
...
function pinWindow(locX, locY, pinLoc)
        
	checkBreak();
        xyWindowSize = srGetWindowSize()
	if pinLoc == PIN_BOTTOM_RIGHT then
	    srSetMousePos(xyWindowSize[0] - 3, xyWindowSize[1] - 3);
        else
            srSetMousePos(3, xyWindowSize[1] - 3);
        end;
        lsSleep(75);
	safeClick(locX, locY, 0);
	lsSleep(75);
        if pinLoc == PIN_BOTTOM_RIGHT then
            safeClick(xyWindowSize[0] - 4, xyWindowSize[1] - 4, 1);
        else
            safeClick(5, xyWindowSize[1] - 10, 1);
        end
	lsSleep(75);
end
...
function dragWindows(loop_count)
  ...
  if clearUI then
    arrangeStashed(nil, true, 372, 125, 300, 5, 5);
  else
    arrangeStashed(nil, nil, 372, 125, 300, 5, 5);
  end
...
stillWeeding = false;
lastWeedTime = 0;
function waitToHarvest()
    lastWeedTime = lsGetTimer();
    stillWeeding = true;
end
function timeToHarvest()
    if not stillWeeding and (lsGetTimer() - lastWeedTime) >= 12000  then
        return true;
    end
    return false;
end
...
function harvestAll(loop_count)
  ...
  local seedWave = 1
  local lastTops = {}
  local seedCount = loopCount;
  local goneWild = {}
  local pickedClean = {}
  if seedAndWeed then
    seedCount = seeds_per_pass;
  end
  while not did_harvest do
    ...
    local tops = findAllImages("ThisIs.png"); --findAllText(thisIs)
    for i = 1, #tops do
      checkBreak()
    ...
    if is_plant then
      ...
      local weeds = findAllImages("weed.png"); --findAllText(weedThis)
      for i = #weeds, 1, -1 do
      ...
      local waters = findAllImages("weed.png"); --findAllText(weedAndWater)
      for i = #waters, 1, -1 do
      ...
      local harvests = {};
      if timeToHarvest() then harvests = findAllImages("harvestFlax.png"); end --findAllText(harvest)
      ...
      --while harvestLeft or not (finish_up == 1 or quit) do
       --lsSleep(50);
       --checkBreak();
       seedsList = findAllImages("harvestSeeds.png"); --findAllText(harvestSeeds)
    ...
    goneWild = findAllImages("goneWild.png");
    pickedClean = findAllImages("pickedClean.png");
    --if numSeedsHarvested >= seeds_per_iter and not is_plant  then
    if ((harvestLeft - (#goneWild + #pickedClean)) <= 0 and is_plant) or (harvestLeft <= 0 and not is_plant) then -- New method in case one or more plants failed and we have less flax beds than expected
      did_harvest = true
      local harvests = findAllImages("harvest.png"); --findAllText(harvest)
      for i = #harvests, 1, -1 do
        lastClick = lastClickTime(harvests[i][0], harvests[i][1])
        if lastClick == nil or lsGetTimer() - lastClick >= CLICK_MIN_WEED then
          clickText(harvests[i])
...
function ripOutAllSeeds(seedCount)
  if disableRip then
    return;
  end
  checkBreak()
  ...
  local byTheRoots = srFindImage("byTheRoots.png");
  while byTheRoots do
    checkBreak()
    safeClick(byTheRoots[0]+3, byTheRoots[1]+3);
    lsSleep(50);
    --safeClick(byTheRoots[0]+3, 5, 1);
    --lsSleep(75);
    srReadScreen();
    byTheRoots = srFindImage("byTheRoots.png");
  end
  --lsSleep(50);
  ...
  while delay_time > (lsGetTimer() - start_time) do
    ...
    if finish_up == 0 and tonumber(loop_count) ~= tonumber(num_loops) then
      ...
      if lsButtonText(lsScreenX - 110, lsScreenY - 90, nil, 100, 0xFFFFFFff, "Rip Plants") then
        ripOutAllSeeds(tonumber(loop_count))
        quit = true
      end
