...
function doit()
  ...
  statusScreen("Closing Windows ...", nil, 0.7);
  center = getCenterPos();
...
function waterThese()
  ...
  if fullAutoMode then
    ...
    statusScreen(fullModeStatusMessage .. "\n\n[" .. tended .. "] Tendings -- All Plants Watered\n\n\nClick Abort button if something went wrong. This will close all veggie windows and start over; BUT you can still continue watering plants OR Let Automode finish watering remaining plants.", nil, 0.7);
    refreshWindows();
...
function pinWindows()
	...
	checkBreak();
	srSetMousePos(bottomRightX, bottomRightY);
	safeClick(vegclickList[i][1], vegclickList[i][2]);
	lsSleep(click_delay);
        safeClick(bottomRightX-5, bottomRightY-5, 1);
	lsSleep(click_delay);
    end
...
function getPlantWindowPos()
  srReadScreen();
  plantPos = srFindImage("plant.png"); --findText(seedName);
  if plantPos then
...
function pickUpSeeds(noClick)
  ...
  lsSleep(75);
  srReadScreen();
  utility = srFindImage("utility"); --findText("Utility");
  lsSleep(200);
	if utility then
	...
	if #vegclickTimer > 0 and #vegclickList > 0 then
	  ...
	  if #vegclickTimer > 0 and #vegclickList > 0 then
	    if ButtonText(10, lsScreenY - 30, z, 175, 0x80ff80ff, "Engage Auto Mode!") then
	      ...
	      sleepWithStatus(1500, "Engaging Auto Water Mode!", nil, 0.7);
	      fm_is_done = 1;
	    --break;
            main();
	    end
