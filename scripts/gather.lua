...
WaypointOrder[#WaypointOrder + 1] = Basket
...
WaypointColors[Warehouse] = {0x562511FF, 0xAD7547FF} -- was 0xA07338FF
WaypointColors[Basket] = 0x893B10FF
...
WaypointColors[SpindleTree] = {0x394314FF, 0x404C15FF}
...
WaypointColors[Khaya] = 0x7A7927FF --0x57570DFF;
WaypointColors[Mimosa] = 0x709B50FF --0x598742FF;
...
function doit()
    if
        (not promptOkay(
            'WARNING! This macro requires your chats to be minimized and your "Use arrow keys for movement" option to be enabled. Enable fast gather from the One-Click Options menu.'
        ))
     then
        return
    end
    if
        (not promptOkay(
            "Time of day, shadows and light intensity should be on lowest setting. Updated for T8 by Manon."
        ))
     then
        return
    end
...
function queryRoute()
    ...
    while not done do
        ...
        y = y + 32
        lime = readSetting("lime", lime)
        lime = lsCheckBox(10, y, z, 0xFFFFFFff, " Gather limestone and soda", lime)
        writeSetting("lime", lime)
        y = y + 32
        ...
        y = y + 32
        stayWaterfull = readSetting("stayWaterfull", stayWaterfull)
        stayWaterfull = lsCheckBox(10, y, z, 0xFFFFFFff, " Gather water always", stayWaterfull)
        writeSetting("stayWaterfull", stayWaterfull)
        if (stayWaterfull) then
            emptyJugs = 2 -- Assume we might start with more than 1 empty jug
        end
        y = y + 36
...
function editRoute(route)
    ...
    while not done do
        ...
        y = y + 32
        done, thisRoute[0] = lsEditBox("routeName" .. unique, 5, y, z, 404, 30, scale, scale, 0x000000ff, thisRoute[0])
        done = nil
        y = y + 50
        lsScrollAreaBegin("routeScrollArea", 5, y, z, 405, 330)
        y = y + 5
        ...
        for i = 1, #thisRoute[1] do
            ...
            nada, t =
                lsEditBox("waypointX_" .. i .. "_" .. unique, x, sy, z + 1, coordw, 30, scale, scale, 0x000000ff, t)
            if (#thisRoute[1] >= i) then
                thisRoute[1][i][1] = t
            end
            x = x + coordw + 2
            ...
            nada, t =
                lsEditBox("waypointY_" .. i .. "_" .. unique, x, sy, z + 1, coordw, 30, scale, scale, 0x000000ff, t)
            if (#thisRoute[1] >= i) then
                thisRoute[1][i][2] = t
            end
            x = x + coordw + 2
            ...
            t = lsDropdown("waypointType_" .. i .. "_" .. unique, x, sy, z, waypointw, t, WaypointTypesOrdered)
            if (#thisRoute[1] >= i) then
        ...
        if (haveMenuClick) then
            ...
            for i = 1, #thisRoute[2] do
                ...
                t = thisRoute[2][i]
                nada, t =
                    lsEditBox("menuText_" .. i .. "_" .. unique, x, sy, z + 1, 300, 30, scale, scale, 0x000000ff, t)
                thisRoute[2][i] = t
                sy = sy + 32
            end
            if (deleteOne) then
        ...
        lsPrintWrapped(
            ...
            1,
            0xFFFFFFff,
            "Tap Ctrl to add a waypoint using your current location."
        )
        local height = #thisRoute[1] + #WaypointTypes + 100
        ...
        if lsButtonText(140, y, z, 90, 0xFFFFFFff, "Duplicate") then
            if (routeText == "") then
                if (not PromptOkay("There's nothing to save.")) then
                    done = false
                end
...
function followRoute(route)
    if (slate or silt or lime or clay or grass or wood or papy) then
        ...
        if (not haveWarehouse) then
            if papy then
                ...
                if
                    (not promptOkay(
                        "This route does not include a warehouse.  If you add a warehouse's coords to this route, resources will be stashed there every time you reach those coordinates."
                    ))
                 then
                    return
                end
        ...
        if (not wood or silt or slate or lime or clay or grass or papy) then
    ...
    while (1) do
        ...
        elseif (r[curr][3] == Warehouse) then
            srReadScreen()
            clickAllImages("deadPinned.png", 5, 5)
            lsSleep(75)
            srReadScreen()
            clickAllImages("ThisIs.png")
            lsSleep(75)
            srReadScreen()
            local stash = srFindImage("stash.png") --findText("Stash...")
            if stash then
                stashAllButWood()
            end
        ...
        elseif (r[curr][3] == Water) then
            if (clay and (emptyJugs > 0)) then
                local waterAttempt = 0
                while (not fillJugs() and (waterAttempt < 3)) do
                    walkTo(r[curr][1] + math.random(-1, 1), r[curr][2] + math.random(-1, 1), false, false)
                    if (not walkTo(r[curr][1], r[curr][2])) then
                        return false
                    end
                    waterAttempt = waterAttempt + 1
                end
            end
        elseif (wood and r[curr][3] ~= Waypoint) then
            if (ensureClickWaypoint(r, curr)) then
                ...
                if (noWoodMenuPos) then
                    ...
                    if carving and not focusMaxed then
...
function clickWaypointPixel(x, y, typeOfWaypoint)
    ...
    local roughness = 20
    if
        (typeOfWaypoint == Oranje) or (typeOfWaypoint == Ranyahn) or (typeOfWaypoint == Hawthorn) or
            (typeOfWaypoint == MiniatureFernPalm)
     then
        roughness = 100
        radius = 4
    end
    ...
    if (pixelBlockCheck(x, y, WaypointColors[typeOfWaypoint], rgbTol, hueTol, roughness, radius)) then
        if (typeOfWaypoint == Warehouse) then
            ...
            --lsSleep(150);
            --srReadScreen();
            --local pos = srFindImage("full.png"); --findText("full");
            --if(not pos) then
            --    lsPrintln("Looking for " .. WaypointTypes[typeOfWaypoint] .. " but clicking didn't bring up a menu.");
            --    checkForMenu();
            --    return false;
            ...
            if
                (pixelBlockCheck(x, y + radius * 2, WaypointColors[typeOfWaypoint], rgbTol, hueTol, roughness, radius) and
                    pixelBlockCheck(
                        x,
                        y - radius * 2,
                        WaypointColors[typeOfWaypoint],
            ...
            if
                (pixelBlockCheck(x - radius * 2, y, WaypointColors[typeOfWaypoint], rgbTol, hueTol, roughness, radius) and
                    pixelBlockCheck(
                        x + radius * 2,
                        y,
                        WaypointColors[typeOfWaypoint],
...
function walkTo(x, y, showStatus, promptIfNotMoving)
    ...
    while (1) do
        updateStatus()
        srReadScreen()
        if (checkWater() or checkSlate() or checkLime() or checkClay() or checkGrass() or checkSilt()) then
            moving = true
            ...
            if (pos[0] == lastPos[0] and pos[1] == lastPos[1]) then
                ...
                if (lsGetTimer() > lastMoveTime + 8000) then
                    if (promptIfNotMoving) then
                        if
                            (not promptOkay(
                                'You don\'t appear to be moving.  Are your chats minimized?  Is your "Use arrow keys for movement" option enabled?'
                            ))
                         then
                            stopMoving()
                            return false
...
function checkLime()
    ...
    local pos = srFindImage("limestone.png", 7000)
    if (pos) then
        stopMoving()
        safeClick(pos[0] + 3, pos[1] + 3)
        sleepWithBreak(1500)
...
function isWithinRange(x, y)
    xyWindowSize = srGetWindowSize()
    local radius = xyWindowSize[0] * 0.75
    local dx, dy, d
    dx = x - getCenterPos()[0]
    dy = y - getCenterPos()[1]
...
function checkSilt()
    ...
    --clusters = lsAnalyzeCustom(90, 220, 0, xyWindowSize[0], 0x8B8378FF, 0x9C948BFF, true);
    --clusters = lsAnalyzeCustom(90, 220, 0, xyWindowSize[0], 0x8B8378FF, 0xa29c90FF, true);
    local clicked = nil
    lsSleep(75)
    srReadScreen()

    local gotSilt = srFindImage("silt.png", 3200)
    if gotSilt then
        stopMoving()
        sleepWithBreak(500)
        srReadScreen()
        --clusters = lsAnalyzeCustom(90, 220, 1, xyWindowSize[0], 0x8B8378FF, 0x9C948BFF, true);
        --sleepWithBreak(1500);
        gotSilt = srFindImage("silt.png", 3100)
        if gotSilt then
            safeClick(gotSilt[0] + 5, gotSilt[1] + 5)
            sleepWithBreak(4000)
            srReadScreen()
        end
...
function plantPapy()
    ...
    srReadScreen()
    local pappy = srFindImage("papyrus.png")
    safeClick(pappy[0] + 5, pappy[1] + 3)
    lsSleep(75)
    srReadScreen()
    local pos = srFindImage("plant.png") --findText("Papyrus");
    if (pos) then
        safeClick(pos[0] + 5, pos[1] + 3)
        sleepWithBreak(75)
        return true
    else
...
function stashAllButWood()
    ...
    while (stashedSomething) do
        ...
        lsSleep(150)
        srReadScreen()
        local pos = srFindImage("stash.png") --findText("Stash...");
        if (pos) then
            ...
            lsSleep(250)
            srReadScreen()
            local pos = srFindImage("invClay.png") --findText("Clay (");
            if (pos) then
                stashItem(pos, true)
                stashedSomething = true
            else
                pos = srFindImage("invFlint.png") --findText("Flint (");
                if (pos) then
                    stashItem(pos, true)
                    stashedSomething = true
                else
                    pos = srFindImage("stashSlate.png") --findText("Slate (");
                    if (pos) then
                        stashItem(pos, true)
                        stashedSomething = true
                    else
                        pos = srFindImage("stashGrass.png") --findText("Grass (");
                        if (pos) then
                            stashItem(pos, true)
                            stashedSomething = true
                        else
                            pos = srFindImage("invTadpoles.png"); --findText("Tadpoles");
                            if (pos) then
                                stashItem(pos, true)
                                stashedSomething = true
                            else
                                pos = srFindImage("stashWood.png") --findText("Wood (");
                                if (pos) then
                                    stashItem(pos, true)
                                    stashedSomething = true
                                else
                                    pos = srFindImage("invLimestone.png") --findText("Limestone (")
                                    if (pos) then
                                        stashItem(pos, true)
                                        stashedSomething = true
                                    else
                                        pos = srFindImage("invSoda.png") --findText("Soda (")
                                        if (pos) then
                                            stashItem(pos, true)
                                            stashedSomething = true
                                        else
                                            ...
                                            if (pos) then
                                                ...
                                                pos = findImage("invInsect.png") --findText("Insect...");
                                                if (pos) then
                                                    safeClick(pos[0] + 10, pos[1] + 5)
                                                    lsSleep(150)
                                                    srReadScreen()
                                                    pos = findImage("allInsect.png") --findText("All Insect");
                                                    if (pos) then
                                                        stashItem(pos, false)
                                                        stashedSomething = true
                                                    end
                                                else
                                                    pos = findImage("invSharpened.png") --findText("Insect...");
                                                    if (pos) then
                                                        stashItem(pos, true)
                                                        stashedSomething = true
                                                    else
                                                        pos = findImage("invTinder.png") --findText("Insect...");
                                                        if (pos) then
                                                            stashItem(pos, true)
                                                            stashedSomething = true
                                                        else
                                                            pos = findImage("invRawhide.png") --findText("Insect...");
                                                            if (pos) then
                                                                stashItem(pos, true)
                                                                stashedSomething = true
                                                            else
                                                                pos = findImage("invPeg.png") --findText("Insect...");
                                                                if (pos) then
                                                                    stashItem(pos, true)
                                                                    stashedSomething = true
                                                                else
                                                                    pos = findImage("invCrude.png") --findText("Insect...");
                                                                    if (pos) then
                                                                        stashItem(pos, true)
                                                                        stashedSomething = true
                                                                    else
                                                                    pos = findImage("invPestle.png") --findText("Insect...");
                                                                    if (pos) then
                                                                        stashItem(pos, true)
                                                                        stashedSomething = true
                                                                    else
                                                                        pos = findImage("invChit.png") --findText("Insect...");
                                                                        if (pos) then
                                                                            safeClick(pos[0] + 10, pos[1] + 5);
                                                                            lsSleep(150)
                                                                            srReadScreen()
                                                                            pos = findImage("allChit.png") --findText("All Insect");
                                                                            if (pos) then
                                                                                stashItem(pos, false)
                                                                                stashedSomething = true
                                                                            end
        ...
        srKeyDown(27); -- clears open windows.
        lsSleep(50);
        srKeyUp(27);
        lsSleep(50);
        count = count + 1
