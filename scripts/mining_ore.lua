...
ruleMode = 0;  -- 0 - standard, 1 - cobalt, 2 - lead, 3 - Platinum

--lsPrintln(inspectit(dropdown_key_values));
...
function TakeOreWindowRefresh()
 clickAllImages("deadPinned.png", 3, 3);
 findAllGems = findImage("allGems.png");
 findAllOre = findImage("allOre.png"); --findText("All Ore");
 findAllMetal = findText("All Metal"); -- Silver Mines give metal, not Ore. This check is for Silver Mines.
 local doDelay = (findAllGems or findAllOre or findAllMetal);
     if not autoWorkMine and doDelay then
	...
	if findAllOre then
	 safeClick(findAllOre[0]-2,findAllOre[1]-2);
	end
        if findAllGems then
	 safeClick(findAllGems[0]-2,findAllGems[1]-2);
	end
	if findAllMetal then
	 safeClick(findAllMetal[0]-2,findAllMetal[1]-2);
	end
...
if not skipWork then
                ...
                lsSleep(clickDelay);
                clickAllText("[A]", 20, 2); -- offsetX, offsetY, rightClick (1 = true)
                ...
                lsSleep(clickDelay);
                clickAllText("[S]", 20, 2); -- offsetX, offsetY, rightClick (1 = true)
...
function getTraits()
  ...
  traits_done = nil;
  trait_value = 1;
  traitsMaxVals[trait] = trait_value;
    ...
    y = 40;
      ...
      for i=1,#clickList do
	    ...
	    if minesB[j][1] == i then
	      lsPrint(150 + ((30 * minesB[j][2]) - 30), y, 0, 0.7, 0.7, 0xFFFFFFff, minesB[j][2] .. ":" .. minesB[j][3]);
	    end
        end --end j
	  ...
	  if ButtonText(110, y, z, 50, 0xFFFFFFff, "Set", 0.7, 0.7) then
	      ...
	      if not isSet then
	        ...
	        mines[index].trait[trait] = trait_value;
                if not traitsMaxVals[trait] then
                    traitsMaxVals[trait] = trait_value;
                else
                    traitsMaxVals[trait] = math.max(traitsMaxVals[trait], trait_value);
                end
                lsPrintln("Node: " .. i .. " " .. trait .. ":" .. trait_value);
	      else
                lsPrintln("NOT SET! " .. i .. " " .. trait .. ":" .. trait_value);
              end
              lsDoFrame()
	      lsSleep(16)
	  end
...
function findSets()
    ...
    local set_max_size = 4; --stonecount;
    -- find all sets
	...
	for set_size = set_min_size, #mines do
        ...
        -- for cobolt and others if manual and some same, some different...
        if ruleMode == 1 then
            set_max_size = 3;
        end
        ...
        local matchSets = {}
        --traitsMaxVals
        lsPrintln("traitsMaxVals: \n" .. inspectit(traitsMaxVals));
        lsPrintln("num_traits: " .. num_traits);
        for curTrait = 1, num_traits do
            matchSets[curTrait] = {};
            for curVal = 1, traitsMaxVals[curTrait] do
                matchSets[curTrait][curVal] = {}
                for curMine = 1, #mines do
                    if mines[curMine].trait[curTrait] == curVal then
                        matchSets[curTrait][curVal][#matchSets[curTrait][curVal]+1] = curMine;
                    end
                end
            end
        end
        for curTrait = #matchSets, 1, -1 do
            for curVal = #matchSets[curTrait], 1, -1 do
                if #matchSets[curTrait][curVal] == 1 then
                    table.remove(matchSets[curTrait], curVal);
                end
            end
        end
...
--i=#input,1,-1 do
        ...
        lsPrintln("matchSets: \n" .. inspectit(matchSets));
        for curTrait = 1, #matchSets do
            for curVal = 1, #matchSets[curTrait] do
                --if #matchSets[curTrait][curVal] > 1 then -- we have 2 or more in this set to match with next traits
                    for i=1, #matchSets[curTrait][curVal] do
                        for j=i+1, #matchSets[curTrait][curVal] do
                            for curMine = 1, #mines do
                                if mines[curMine].trait[curTrait] ~= curVal and
                                   curMine ~= i and curMine ~= j then
                                    potentialSets[#potentialSets + 1] = { i, j, curMine };
                                    table.sort(potentialSets[#potentialSets]);
                                end
                            end
                        end
                    end
        ...
        table.sort(potentialSets, function(a, b)
           if(a[1] < b[1]) then
               return true;
           elseif (a[1] == b[1]) then
               if (a[2] < b[2]) then
        ...
        for curSet = #potentialSets, 1, -1 do
            if curSet ~= 1 and
               potentialSets[curSet][1] == potentialSets[curSet - 1][1] and
               potentialSets[curSet][2] == potentialSets[curSet - 1][2] and 
               potentialSets[curSet][3] == potentialSets[curSet - 1][3] then
                   table.remove(potentialSets, curSet);
            end
        end

        for curSet = #potentialSets, 1, -1 do
            if not is_matching_set(potentialSets[curSet]) then
                table.remove(potentialSets, curSet);
            end
        end 
        
        lsPrintln(inspectit(potentialSets));
        sets = potentialSets
...
function is_matching_set(set)
    ------DREGER - changed to handle cobolt.
    if ruleMode == 0 then
	...
	return true;
        ...
        lsPrintln("num_traits: " .. num_traits .. " #mines: " .. #mines .. " #set: " .. #set);
	for i=1, num_traits do
            if mines[set[1]].trait[i] == mines[set[2]].trait[i] and
               mines[set[1]].trait[i] == mines[set[3]].trait[i] then
                   if not set[4] then
                       return false;
                   else
                       if mines[set[1]].trait[i] == mines[set[4]].trait[i] then
                         return false;
                       end
                   end
            end
            if mines[set[1]].trait[i] ~= mines[set[2]].trait[i] and
               mines[set[1]].trait[i] ~= mines[set[3]].trait[i] and
               mines[set[2]].trait[i] ~= mines[set[3]].trait[i] then
                   if not set[4] then
                       return false;
                   else
                       if mines[set[4]].trait[i] ~= mines[set[1]].trait[i] and
                          mines[set[4]].trait[i] ~= mines[set[2]].trait[i] and
                          mines[set[4]].trait[i] ~= mines[set[3]].trait[i] then
                           return false;
                       end
                   end
            end
...
function allMinesHaveTrait(trait_num)
	for i=1, #mines do
		if not mines[i].trait[trait_num] then
                        lsPrintln("Node# " .. i .. " Trait# " .. trait_num .. " has no value assigned.");
			return false;
		end
	end
	return true;
